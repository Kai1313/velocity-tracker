...
- `GET /dashboard/sprint-health` — one row per `Open` sprint, aggregated across every `Active` project's work in it, comparing the velocity still required to finish committed work against the velocity actually being achieved, as an early-warning `OnTrack`/`AtRisk`/`Critical` status. See [ADR-0010](docs/adr/0010-project-sprint-health.md) and [ADR-0011](docs/adr/0011-sprint-health-aggregates-across-projects.md).
...
`/dashboard` shows all sprints with workload/done totals, plus a collapsible "Sprint Health" card (`SprintHealthCard`/`SprintHealthTable`, `frontend/components/dashboard/sprint-health-*.tsx`) above the workload/done chart, backed by `GET /dashboard/sprint-health`. It's a sprint-level early-warning table — one row per `Open` sprint, with Committed/Done/Late-Add SP summed across every `Active` project's work in that sprint, days remaining, required vs. achieved velocity, and an `OnTrack`/`AtRisk`/`Critical` status badge. Deliberately not split out per project, since the team's developers share capacity across projects (see [ADR-0011](docs/adr/0011-sprint-health-aggregates-across-projects.md)). Collapsed/expanded state persists via `localStorage`. See [ADR-0010](docs/adr/0010-project-sprint-health.md) for how the status is computed and why the thresholds are still provisional.

Clicking a sprint in the sprints table links to `/dashboard/{sprintId}` for its per-developer breakdown. From there, the "Sprint workload" card links to `/dashboard/{sprintId}/entries`, which splits that sprint's workload into "Current sprint workload" and "Carry-over sprint workload" totals (points and a done/total ticket ratio), each backed by `GET /dashboard/sprints/{id}/entries`'s `current`/`carriedOver` split and excluding Cancelled entries, consistent with how "workload" is computed everywhere else in this app. Built with [shadcn/ui](https://ui.shadcn.com) (`Card`/`Table`/`Badge`) — component source lives in `frontend/components/ui/`, not an npm package, so it can be edited directly. To add more shadcn components later: `npx shadcn@latest add <component>` from `frontend/`.
