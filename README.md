...
`/dashboard` shows all sprints with workload/done totals, plus a collapsible "Sprint Health by Project" card (`SprintHealthCard`/`SprintHealthTable`, `frontend/components/dashboard/sprint-health-*.tsx`) above the workload/done chart, backed by `GET /dashboard/project-sprint-health`. It's a project-level early-warning table, not a per-sprint or per-developer view — one row per `Active` project per `Open` sprint it has work in, with Committed/Done/Late-Add SP, days remaining, required vs. achieved velocity, and an `OnTrack`/`AtRisk`/`Critical` status badge. Collapsed/expanded state persists via `localStorage`. See [ADR-0010](docs/adr/0010-project-sprint-health.md) for how the status is computed and why the thresholds are still provisional.
