...
_Invariant_: a ticket can have at most one `SprintEntry` in an `Open` sprint at a time — enforced by a database trigger, not the service layer. See [ADR-0007](docs/adr/0007-single-open-sprintentry-per-ticket.md).
...
_Reopening_: a `Closed` sprint can be set back to `Open` with no restrictions — a deliberate escape hatch for correcting mistakes (e.g. a ticket registered under the wrong sprint), not a bug. See [ADR-0009](docs/adr/0009-sprint-reopening-is-an-escape-hatch.md).
...
**Sprint Health**:
A per-`Sprint` early-warning signal, computed on the fly (not stored) for every currently `Open` sprint, aggregated across every `Active` project's work in it — automates a report the team previously maintained by hand. Compares **Required Velocity** (`(Committed SP − Done SP) ÷ Days Remaining`) against **Achieved Velocity** (`Done SP ÷ Days Elapsed`), both using committed-only points, to produce an `OnTrack`/`AtRisk`/`Critical` status. Deliberately not broken out per project: this is one team whose developers work across projects, so a project-level split would distort the pace comparison rather than clarify it. See [ADR-0010](docs/adr/0010-project-sprint-health.md) and [ADR-0011](docs/adr/0011-sprint-health-aggregates-across-projects.md); the `AtRisk`/`Critical` ratio cutoffs are a placeholder for v1, not a validated threshold.
